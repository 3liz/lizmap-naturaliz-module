ALTER TABLE taxon.t_complement RENAME COLUMN menace TO menace_nationale;
ALTER TABLE taxon.t_complement ADD COLUMN menace_regionale character varying(5);
UPDATE taxon.t_complement SET menace_regionale = menace_nationale WHERE menace_regionale IS NULL;
...
DROP MATERIALIZED VIEW IF EXISTS taxon.taxref_consolide_non_filtre CASCADE;
...
CREATE OR REPLACE VIEW occtax.vm_stat_nb_observations_par_groupe_taxonomique AS
SELECT
