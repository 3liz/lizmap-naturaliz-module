...
WITH s AS (
    ...
    agg_validateur AS (
        SELECT
            ...
            string_agg( concat(
                'Validation ' ||  (ARRAY['producteur', 'régionale', 'nationale'])[ech_val::integer] || ': ',
                identite,
                CASE
                    WHEN organisme IS NULL OR organisme = '' THEN ''
                    ELSE ' (' || organisme|| ')'
                END
            ), ', ' ORDER BY identite) AS validateur
        FROM occtax.v_validateurs
        GROUP BY identifiant_permanent
    ),
    ...
    SELECT
    o.cle_obs,
    lm01.code_maille AS code_maille_01, lm01.code_maille_unique AS code_maille_01_unique,
    lm02.code_maille AS code_maille_02, lm02.code_maille_unique AS code_maille_02_unique,
    lm10.code_maille AS code_maille_10, lm10.code_maille_unique AS code_maille_10_unique,
    ...
    LEFT JOIN agg_validateur    AS pval  ON pval.identifiant_permanent = o.identifiant_permanent
