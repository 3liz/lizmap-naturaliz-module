...
---------------------------------------------------------------------------
Finalisé le 11/08/2020 par Valentin Le Tellier à partir des fichiers type de Michaël Douchin :
...
4. Mise à jour de la table des statuts t_complement 
	...
	4.4 Mise à jour du champ menace
	4.5 Mise à jour du champ menace_monde
	4.6 Mise à jour du champ protection
	4.7 Mise à jour du champ det_znieff
	4.8 Mise à jour optionnelle du champ invasibilité
...
-- 4.4 Mise à jour des champs menace et menace_monde (protection, det_znieff)
-----------------------------
-- TODO : compélter pour menace_region lorsque le champ aura été créé par MD

/* On n'effectue plus les parties suivantes, conservées pour mémoire, car :
- à partir de la v12 de la base de statuts, tout est dans une seule table, intégrant les menaces ;
- désormais le premier INSERT dans t_complement se base sur taxref UNION taxref_local (et plus seulement taxref_valide).

TRUNCATE menaces RESTART IDENTITY;
COPY menaces
FROM '{$menace}' DELIMITER ',' CSV;

COMMENT ON TABLE taxon.menaces
    IS 'Données sur les menaces, issues des listes rouges'; --todo : vérifier avec MD si cette table est utile et si non, la supprimer 
...
WITH s AS (
    ...
    FROM taxon.taxref t
    INNER JOIN (
			SELECT FROM taxon.statuts_especes_12 WHERE lb_adm_tr IN ('Réunion') AND lb_type_statut IN ('Liste rouge nationale')
				) ON m.cd_nom = t.cd_nom OR m.cd_nom = t.cd_ref OR m.cd_ref = t.cd_ref
			)
...
SET menace = synthese_menace.code_statut
...
/* On n'effectue plus les parties suivantes, conservées pour mémoire, car :
- à partir de la v12 de la base de statuts, tout est dans une seule table, intégrant les protections ;
- désormais le premier INSERT dans t_complement se base sur taxref UNION taxref_local (et plus seulement taxref_valide).
...
WITH ss AS (
    ...
    CASE
        WHEN p.cd_protection IN ('NMAMmar2', 'NMAMmar3', 'NMAMmar5', 'NTM1', 'NTM8', 'REUEA2', 'REUEA3', 'REUEA4', 'REUI2', 'VP974') THEN 'EPN' --'NMAMmar3', 'NMAMmar5' ajoutés par rapport à la v11
        --WHEN p.cd_protection IN ({$code_arrete_protection_communautaire}) THEN 'EPC' -- finalement, les textes UE ne sont pas applicables à La Réunion -- todo : retirer du menu déroulant ?
        WHEN p.cd_protection IN ('IAO3', 'IBO1', 'IBO2', 'IBOAC', 'IBOAE', 'IBOAS', 'IBOAW', 'IBOC', 'IBOEU') THEN 'EPI' -- 'IBOAC', 'IBOAS', 'IBOAW', 'IBOC', 'IBOEU' ajoutés par rapport à la V11
        WHEN p.cd_protection IN ('agri1', 'agri2', 'Bubul1', 'Bulbul2', 'Bulbul3', 'Bulbul4', 'Bulbul5', 'Bulbul6', 'Bulbul9', 'corbasi1', 'phelsuma1','phelsuma2', 'phelsuma3', 'phelsuma4', 'phelsuma5', 'PV97', 'REUEEA', 'REUEEI', 'REUnoEEEA', 'REUnoEEEA2','REUnoEEEV', 'REUP') THEN 'EPA' --'REUnoEEEA', 'REUnoEEEA2','REUnoEEEV' ajoutés par rapport à la v11
        ELSE NULL
    END AS "protection",
    CASE
        WHEN p.cd_protection IN ('NMAMmar2', 'NMAMmar3', 'NMAMmar5', 'NTM1', 'NTM8', 'REUEA2', 'REUEA3', 'REUEA4', 'REUI2', 'VP974') THEN 0
        --WHEN p.cd_protection IN ({$code_arrete_protection_communautaire}) THEN 1
        WHEN p.cd_protection IN ('IAO3', 'IBO1', 'IBO2', 'IBOAC', 'IBOAE', 'IBOAS', 'IBOAW', 'IBOC', 'IBOEU') THEN 2
        WHEN p.cd_protection IN ('agri1', 'agri2', 'Bubul1', 'Bulbul2', 'Bulbul3', 'Bulbul4', 'Bulbul5', 'Bulbul6', 'Bulbul9', 'corbasi1', 'phelsuma1','phelsuma2', 'phelsuma3', 'phelsuma4', 'phelsuma5', 'PV97', 'REUEEA', 'REUEEI', 'REUnoEEEA', 'REUnoEEEA2','REUnoEEEV', 'REUP') THEN 3
        ELSE NULL
    END AS "note"
    ...
    AND p.cd_protection IN (
        'NMAMmar2', 'NMAMmar3', 'NMAMmar5', 'NTM1', 'NTM8', 'REUEA2', 'REUEA3', 'REUEA4', 'REUI2', 'VP974',
        'IAO3', 'IBO1', 'IBO2', 'IBOAC', 'IBOAE', 'IBOAS', 'IBOAW', 'IBOC', 'IBOEU',
        'agri1', 'agri2', 'Bubul1', 'Bulbul2', 'Bulbul3', 'Bulbul4', 'Bulbul5', 'Bulbul6', 'Bulbul9', 'corbasi1', 'phelsuma1','phelsuma2', 'phelsuma3', 'phelsuma4', 'phelsuma5', 'PV97', 'REUEEA', 'REUEEI', 'REUnoEEEA', 'REUnoEEEA2','REUnoEEEV', 'REUP'
...
DROP TABLE taxon.protections ; --TODO : vérifier avec MD
DROP TABLE taxon.menaces ; --TODO : vérifier avec MD
